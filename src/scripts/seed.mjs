...
function field(text, label) {
	const m = text.match(
		new RegExp(`<small>${label}</small>.*?<p[^>]*>\\s*([^<]+?)\\s*</p>`, "s"),
	);
...
const boxes = descargasHtml
	.split('<div class="searchest-lines__box">')
	.slice(1)
	.map((b) => {
...
for (const b of boxes)
	console.log(`   - ${b.name} | ${b.origin} -> ${b.destChain}`);

const origenSelect = descargasHtml.match(
	/<select[^>]*id="origenSelect".*?<\/select>/s,
);
if (!origenSelect)
	throw new Error("No se encontró el select de origen en /descargas");
const normalize = (s) =>
	s
		.normalize("NFD")
		.replace(/[\u0300-\u036f]/g, "")
		.toLowerCase();
const idMap = new Map();
for (const m of origenSelect[0].matchAll(
...
for (const box of boxes) {
	...
	if (!originId || !destId) {
		console.log(
			`   [skip] ${box.name}: sin id para ${box.origin}(${originId}) o ${dest}(${destId})`,
		);
		continue;
	}
	console.log(
		`   POST ${box.name}: ${box.origin}(${originId}) <-> ${dest}(${destId})`,
	);
	for (const direction of [
		...
		for (const chunk of html.split('<div class="row" id="').slice(1)) {
			...
			for (const piece of chunk.split("well-info custom-row").slice(1)) {
				...
				const latlon = piece.match(
					/data-latitud="([\d.]+)" data-longitud="([\d.]+)"/,
				);
				stops.push({
					town,
					name,
			...
			if (!lines.has(lineId))
				lines.set(lineId, { name: box.name, pdf: box.pdf });
			schedules.push({
				lineId,
				originTown,
				destTown,
...
for (const [lineId, line] of lines) {
	try {
		const resp = await fetch(`${BASE}/descargas-pdf-linea/${lineId}`, {
			redirect: "manual",
		});
		...
		console.warn(
			`   [pdf] no se pudo resolver para la línea ${lineId}: ${e.message}`,
		);
	}
}
...
const counts = await db.execute(
	"SELECT (SELECT count(*) FROM lines) AS l, (SELECT count(*) FROM schedules) AS s",
);
console.log("4) verificación:", counts.rows[0]);

for (const [id, l] of lines)
	console.log(`   línea ${id}: ${l.name} -> ${l.pdf}`);
